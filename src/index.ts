...
export default class ApiWorker extends WorkerEntrypoint<Env> {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === '/persona/chat/ws' && request.headers.get('Upgrade') === 'websocket') {
      const persona = url.searchParams.get('persona') ?? '';
      const id = this.env.PERSONA_CHAT_ROOM.idFromName(persona.toLowerCase());
      const stub = this.env.PERSONA_CHAT_ROOM.get(id);
